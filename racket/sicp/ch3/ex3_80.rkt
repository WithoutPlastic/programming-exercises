...
(define [generic-solve-2nd f a b dt y0 dy0]
  (define y (integral (lambda [] dy) y0 dt))
...
(define [make-RLC R L C dt]
  (lambda [vC0 iL0]
    (define vC (integral (lambda [] dvC) vC0 dt))
    (define iL (integral (lambda [] diL) iL0 dt))
    (define dvC (scale-stream iL (/ -1.0 C)))
    (define diL (add-streams (scale-stream vC (/ 1.0 L))
                             (scale-stream iL (- (/ R L)))))
    (alt-stream-map (lambda [x y] (cons x y))
                    vC
                    iL)))

(display-stream ((make-RLC 1 1 0.2 0.1) 10 0))
