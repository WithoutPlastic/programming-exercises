...
(define [ripple-carry-adder addend-a addend-b sum-c in-cin out-cout]
  (define [make-ripple-carry-adder remaining-addend-a
                                   remaining-addend-b
                                   remaining-sum-c
                                   inner-in-cin]
    (let ([inner-out-cout (make-wire)])
      (if [or (null? remaining-addend-a)
              (null? remaining-addend-b)
              (null? remaining-sum-c)]
        (set-signal! inner-out-cout inner-in-cin)
        (begin
          (full-adder (car remaining-addend-a)
                      (car remaining-addend-b)
                      (car remaining-sum-c)
                      inner-in-cin
                      inner-out-cout)
          (make-ripple-carry-adder (cdr remaining-addend-a)
                                   (cdr remaining-addend-b)
                                   (cdr remaining-sum-c)
                                   inner-out-cout)))))

  (if [= (length addend-a) (length (addend-b)) (length sum-c)]
    (make-ripple-carry-adder addend-a addend-b sum-c in-cin)
    (error "two addend signal bits not equal -- RIPPLE-CARRY-ADDER")))

; Delay is sum of all full adder delay
