...
(define partial-sums
  (cons-stream 1
               (lambda [] (add-streams partial-sums
                                       (integers-starting-from 2)))))

(define [merge stream-a stream-b]
  (cond ([empty-stream? stream-a] stream-b)
        ...
        (else
          (let ([first-elt-a (stream-car stream-a)]
            ...
            (cond ([< first-elt-a first-elt-b]
                   (cons-stream first-elt-a
                                (lambda []
                                  (merge (stream-cdr stream-a) stream-b))))
                  ([< first-elt-b first-elt-a]
                   (cons-stream first-elt-b
                                (lambda []
                                  (merge (stream-cdr stream-b) stream-a))))
                  (else
                    (cons-stream first-elt-a
                                 (lambda []
                                   (merge (stream-cdr stream-a)
                                          (stream-cdr stream-b))))))))))

;### INCORRECT ANSWER ###
;(define twos (cons-stream 2 (lambda [] twos)))
;(define threes (cons-stream 3 (lambda [] threes)))
;(define fives (cons-stream 5 (lambda [] fives)))
;(define two-exp-stream
;  (cons-stream 1 (lambda [] (mul-streams twos two-exp-stream))))
;(define three-exp-stream
;  (cons-stream 1 (lambda [] (mul-streams threes three-exp-stream))))
;(define five-exp-stream
;  (cons-stream 1 (lambda [] (mul-streams fives five-exp-stream))))
;(define merge-235-numbers
;  (cons-stream 1
;               (lambda [] (merge two-exp-stream
;                                 (merge three-exp-stream
;                                        five-exp-stream)))))
;### INCORRECT ANSWER ###
