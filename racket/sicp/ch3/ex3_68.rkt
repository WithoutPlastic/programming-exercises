...
(define [interleave stream-a stream-b]
  (cond ([empty-stream? stream-a] stream-b)
        ...
        (else (cons-stream (stream-car stream-a)
                           (lambda [] (interleave stream-b stream-a))))))
    ...
    (lambda []
      (interleave
        (stream-map (lambda [x] (list (stream-car stream-s) x))
                    (stream-cdr stream-t))
        (pairs (stream-cdr stream-s) (stream-cdr stream-t))))))
(define [problem-pairs stream-s stream-t]
  (interleave
    (stream-map (lambda [x] (list (stream-car stream-s) x))
                stream-t)
    (pairs (stream-cdr stream-s) (stream-cdr stream-t))))

;answer:the problematic pairs will call interleave -> pairs -> interleave in
;infinite loop. And the original pairs will paused in cons-stream call, the
;next term is delayed via the lambda expression
