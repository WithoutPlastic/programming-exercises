...
;two integers stream with i <= j and sorted by i + j
(weighted-pairs integers
                integers
                (lambda [pair] (+ (car pair) (cadr pair))))
;two integers can be divided by any of 2 3 5
;and sorted by 2i + 3j + 5ij
(stream-filter (lambda [pair]
                 [andmap
                   (lambda [x]
                     [ormap
...
;simplified solution
(define i-235
  (stream-filter (lambda [x] (ormap (lambda [y] (= (remainder x y) 0))
                                    (list 2 3 5)))
                 integers))
(weighted-pairs i-235
                i-235
                (lambda [pair]
                  (+ (* 2 (car pair))
                     (* 3 (cadr pair))
