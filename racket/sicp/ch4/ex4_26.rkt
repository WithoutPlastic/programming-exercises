...
(define [make-unless-expr predicate alternative exceptional]
  (list 'unless predicate alternative exceptional))
(define [unless-predicate unless-expr] (car (tagged-expr-body unless-expr)))
(define [unless-alternative unless-expr] (cadr (tagged-expr-body unless-expr)))
(define [unless-exceptional unless-expr] (caddr (tagged-expr-body unless-expr)))
(define [unless->if-expr unless-expr]
  (let ([predicate (unless-predicate unless-expr)]
        [alternative (unless-alternative unless-expr)]
        [exceptional (unless-exceptional unless-expr)])
    (make-if predicate exceptional alternative)))
...
;(a b c d e f)
;
;Answer: About the syntax special form and procedure, according to hints, when
;used in high order process, syntax special form not work. And process form is
;always recommended.
