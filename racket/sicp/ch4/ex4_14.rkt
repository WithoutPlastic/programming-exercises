...
(define [make-primitive implementation] (cons 'primitive implementation))
(define [primitive-procedure? tagged-expr]
  [tagged-expr-tag-eq? tagged-expr 'primitive])
(define [primitive-implementation tagged-expr] (cadr tagged-expr))
...
(define primitive-procedure-objects
  (map (lambda [primitive-pair] (make-primitive (cdr primitive-pair)))
...
(define [lookup-variable-value variable env]
  (env-variable-operation
    ...
    (lambda [frame] (error "unbound variable -- LOOKUP-VARIABLE-VALUE" variable))))
(define [set-variable-value! variable new-value env]
  (env-variable-operation
    variable
    env
    (lambda [pair] (set-cdr! pair new-value))
    (lambda [frame] (error "set unbound variable failed -- SET-VARIABLE-VALUE!"
                           variable))))
...
(define [make-unbound! variable env]
  ...
  (define [not-founded-error]
      (error "try to unbound variable failed, not existed -- MAKE-UNBOUND!"
             variable))
  (define [frame-iter enclosing-env]
    (if [eq? enclosing-env the-empty-environment]
      (not-founded-error)
      (inner-frame-iter (first-frame enclosing-env))))
  (define [inner-frame-iter var-val-pairs]
    (cond ([null? var-val-pairs] (not-founded-error))
          ([eq? (car (first-pair var-val-pairs)) variable]
           (set-car! (first-pair var-val-pairs) '())
...
(define [apply-primitive-procedure procedure args]
  (apply-in-underlying-racket (primitive-implementation procedure args)))
...
;Answer: I don't quite catch the point the problem. AFAIK, type map definition
;in interaction prompt is to define user compound procedure. It will be added
;in the current frame, and override system default one. It works well.
;
;But what the other case? Since there is no source code, I guess the map is
;present like '(map proc args), a tagged list. I definitely not work, because
;it is not a primitive tagged list, which can't be catch by primitive procedure
;detector.
