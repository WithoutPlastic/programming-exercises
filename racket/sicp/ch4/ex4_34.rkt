...
(define [lcons-expr? expr] [and [pair? expr] [eq? (car expr) 'lcons]])
(define lcons (lambda [x y] (cons 'lcons (lambda [m] (m x y)))))
(define lcar (lambda [z] ((cdr z) (lambda [p q] p))))
(define lcdr (lambda [z] ((cdr z) (lambda [p q] q))))
(define primitive-table
  (list (make-vv-pair 'car lcar)
        (make-vv-pair 'cdr lcdr)
        (make-vv-pair 'cons lcons)
...
(define primitive-procs (filter (lambda [x] [not [eq? x lcons]])
                                (map vv-pair-value primitive-table)))
...
(define [user-print object]
  (define max-lcons-display-elts 5)
  (define skip-notation ".....)")
  (define [display-lcons]
    (define [iter remaining cnt]
      ;(display "####")(display remaining)(display "####")(newline)
      (cond ([= cnt 0] (display skip-notation))
            ([not [lcons-expr? remaining]]
             (user-print remaining)
             (display ")"))
            (else (user-print (force-it (lcar remaining)))(display " ")
                  (iter (force-it (lcdr remaining)) (- cnt 1)))))

      ;(cond ([= cnt 0] (display skip-notation))
      ;      ([number? (force-it (cdr remaining))] (display "END"))
      ;      (else (user-print (force-it (lcar remaining)))
      ;            (iter (force-it (lcdr remaining)) (- cnt 1)))))
    (display "(")
    (iter object max-lcons-display-elts))
  (if [lcons-expr? object]
    (display-lcons)
    (display object)))
(define [driver-loop]
