...
(define [arg-lazy-option? arg]
  [and [pair? arg] [or [eq? (cadr arg) 'lazy] [eq? (cadr arg) 'lazy-memo]]])
(define [make-nonm-thunk expr env] (cons 'nonm-thunk (cons expr env)))
(define [nonm-thunk? object]
  [and [pair? object] [eq? (car object) 'nonm-thunk]])
(define [nonm-thunk-expr nonm-thunk] (cadr nonm-thunk))
(define [nonm-thunk-env nonm-thunk] (cddr nonm-thunk))

(define [make-memo-thunk expr env] (cons 'memo-thunk (cons expr env)))
(define [memo-thunk? object]
  [and [pair? object] [eq? (car object) 'memo-thunk]])
(define [memo-thunk-expr memo-thunk] (cadr memo-thunk))
(define [memo-thunk-env memo-thunk] (cddr memo-thunk))
...
(define [delay-it object env]
  (cond ([not [or [memo-thunk? object] [nonm-thunk? object]]]
         (make-memo-thunk object env))
        (else object)))
(define [delay-all objects env] (map (lambda [x] (delay-it x env)) objects))

(define [force-it object]
  (define [thunk->value] ((memo-thunk-expr object) (memo-thunk-env object)))
  (define [thunk->evaluated!]
    (let ([value ((memo-thunk-expr object) (memo-thunk-env object))])
      (set! object (make-evaluated-thunk value))
      value))
  (define [return-value] (evaluated-thunk-value object))
  
  (cond ([nonm-thunk? object] (thunk->value))
        ([memo-thunk? object] (thunk->evaluated!))
...
(define [analyze-primitive-procedure proc-expr]
  (let ([proc-name (analyze (proc-name proc-expr))]
    ...
    (lambda [env]
      (apply (proc-name env)
             (map (lambda [arg]
                    (if [or [memo-thunk? arg] [nonm-thunk? arg]]
                      (force-it arg)
                      (arg env)))
                  proc-arguments)))))
(define [analyze-compound-procedure proc-expr]
  (let* ([bypass-lazy-option (lambda [x] (if [arg-lazy-option? x]
                                          (cons (analyze (car x)) (cadr x))
                                          (analyze x)))]
         [proc-name (analyze (proc-name proc-expr))]
         [proc-arguments (map bypass-lazy-option (proc-arguments proc-expr))])
    (lambda [env]
      (apply (proc-name env)
             (map (lambda [arg] (cond ([eq? (cdr arg) 'lazy]
                                       (make-nonm-thunk arg env))
                                      ([eq? (cdr arg) 'lazy-memo]
                                       (make-memo-thunk arg env))
                                      (else (arg env)))) proc-arguments)))))
