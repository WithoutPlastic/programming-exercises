...
(define [singletion-stream x] (stream-cons x the-empty-stream))
...
;(define [rename-query-symbol tagged-expr]
;  (if [query-symbol-expr? tagged-expr]
;    (make-query-symbol-expr (rename-symbol (query-symbol-expr-symbol tagged-expr)))
;    tagged-expr))
;(define [tree->renamed-query-symbol-tree tree]
;  (tree-map tree rename-query-symbol))
...
;               input->lisp-value-pattern-expr
;               input->conjunction-pattern-expr
;               input->disjunction-pattern-expr
;               input->negate-pattern-expr)])
;
...
;      [input-lisp-value-pattern-expr input]
;      [input-conjunction-pattern? input]
;      [input-disjunction-pattern? input]
;      [input-negate? input]])
;
...
;    (get-indexed-constnat (constant-symbol-expr-symbol domain))
...
;(register-tagged-expr-qeval 'add-assert!-expr qeval-add-assert!-expr expr)
;
;;Add rule expr
...
;(register-tagged-expr-qeval 'add-rule!-expr qeval-add-rule!-expr expr)
;
;;Pattern match
...
;    (let* ([aliased-rule (tree->renamed-query-symbol-tree rule)]
...
;(define [make-conjunction-pattern-expr patterns]
...
;(define [input-disjunction-pattern? input]
;  [and [list? input] [< 2 (length input)]
;       [eq? (car input) 'or]
;       [andmap input-pattern? (cdr input)]])
;(define [input->disconjunction-expr input]
;  [and [input-disjunction-pattern? input]
;       (make-disjunction-expr (map pattern-input->tagged-expr (cdr input)))])
;(add-input->tagged-expr-proc input->disconjunction-expr)
